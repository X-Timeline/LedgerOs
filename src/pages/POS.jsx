    ...
    <div className="w-full flex flex-col lg:flex-row" style={{ backgroundColor: C.bg, fontFamily: "Inter, sans-serif" }}>
      ...
      {showHistory && (
        <div className="fixed inset-0 z-50 flex items-end lg:items-center justify-center px-0 lg:px-4">
          ...
          <div className="relative w-full lg:max-w-lg bg-white rounded-t-3xl lg:rounded-3xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: C.border }}>
              <span className="text-sm font-semibold text-slate-900">Sales History — {selectedShop.name}</span>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-3 space-y-3">
              {salesLoading ? (
                ...
                sales.map((s) => (
                  <div key={s.id} className="border rounded-2xl px-4 py-3" style={{ borderColor: C.border }}>
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[12px] text-slate-400">
                        {new Date(s.created_at).toLocaleString()} · {s.channel}
                        {s.customers?.name ? ` · ${s.customers.name}` : ""}
                      </span>
                      <span className="text-[13px] font-semibold tabular-nums text-slate-900">{money(s.total_amount)}</span>
                    </div>
                    <div className="space-y-1.5">
                      {(s.sale_lines || []).map((line) => (
                        <div key={line.id} className="flex items-center justify-between text-[12px]">
                          <span className="text-slate-600">{productName(line.product_id)} · {line.quantity} {line.unit_sold}</span>
                          <button
                            onClick={() => { setSaleReturnLine({ line, saleId: s.id }); setReturnError(""); }}
      ...
      {/* ---------- Return a sale line modal ---------- */}
      {saleReturnLine && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-slate-900/40" onClick={() => setSaleReturnLine(null)} />
          <div className="relative w-full max-w-sm bg-white rounded-3xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-900">Return item</h3>
              <button onClick={() => setSaleReturnLine(null)}><X size={16} className="text-slate-400" /></button>
            </div>
            <p className="text-[12px] text-slate-500 mb-4">
              {productName(saleReturnLine.line.product_id)} · sold {saleReturnLine.line.quantity} {saleReturnLine.line.unit_sold}
            </p>

            {returnError && (
              <div className="mb-3 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{returnError}</div>
            )}
            ...
            <input
              type="number"
              value={saleReturnQty}
              onChange={(e) => setSaleReturnQty(e.target.value)}
              placeholder={`up to ${saleReturnLine.line.quantity}`}
